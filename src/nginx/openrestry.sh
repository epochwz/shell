...
init(){
    ...
    # 版本名称
    VERSION_NAME=openresty-$NGINX_VERSION
    # 安装包名称
    ZIP_NAME=$VERSION_NAME.tar.gz
    # 安装包下载链接
    DOWNLOAD_URL=https://openresty.org/download/${ZIP_NAME}
    # 安装包下载路径
    DOWNLOAD_PATH=$PACKAGE_PATH/$ZIP_NAME
    # 安装路径
    SETUP_PATH=$SOFTWARE_PATH/$VERSION_NAME
    # 启动路径
    STARTUP=$SETUP_PATH/nginx/sbin/nginx
    # 配置文件路径
    CONF_FILE=$SETUP_PATH/nginx/conf/nginx.conf
    # 域名配置文件路径
    VHOST_PATH=$SETUP_PATH/nginx/conf/vhost
    # PID_NAME
    PID_NAME=$SETUP_PATH/nginx/sbin/nginx
